...
async fn create_ramdisk() -> Result<String, String> {
	//check if the ramdisk already exists and has been used by Fox this session
...
async fn combine_shards() -> Result<String, String> {
	...
	let combine_script = std::path::Path::new(&(get_home().unwrap()+"/fox/combine-shards.sh")).exists();
	if combine_script == true{
		let output = Command::new("sudo").args(["rm", &(get_home().unwrap()+"/fox/combine-shards.sh")]).output().unwrap();
		if !output.status.success() {
	...
	let file = File::create(&(get_home().unwrap()+"/fox/combine-shards.sh")).unwrap();
	...
	let output = Command::new("bash")
		.args([get_home().unwrap()+"/fox/combine-shards.sh"])
		.output()
...
#[tauri::command]
async fn enable_webcam_qr_scan() -> Result<String, String> {
	let script = std::path::Path::new(&(get_home().unwrap()+"/fox/enable-webcam-scan.sh")).exists();
	if script == false{
		//create enable-webcam-scan.sh script
		let file = File::create(&(get_home().unwrap()+"/fox/enable-webcam-scan.sh")).unwrap();
	...
	let output = Command::new("bash").args([&(get_home().unwrap()+"/fox/enable-webcam-scan.sh")]).output().unwrap();
	if !output.status.success() {
